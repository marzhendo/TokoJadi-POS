<x-app-layout>
    ...
    <div class="flex justify-between items-end mb-lg">
        <div>
            <nav class="flex items-center gap-sm text-xs text-text-secondary mb-sm">
                ...
                <span class="material-symbols-outlined" style="font-size:14px">chevron_right</span>
                <span class="text-primary font-bold">Edit Produk</span>
            </nav>
            <h2 class="font-headline text-headline-lg text-text-primary">Edit Produk: {{ $produk->nama }}</h2>
        </div>
    </div>

    <div class="bg-surface-container-lowest rounded-2xl border border-table-border shadow-sm max-w-4xl mb-xl">
        <form action="{{ route('produk.update', $produk) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-6 mb-8">
                ...
                <div>
                    ...
                    <select name="satuan_dasar_id" id="satuan_dasar_select" required class="w-full px-4 py-3 bg-surface-container-low border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-all">
                        ...
                        @foreach($satuanList as $s)
                            <option value="{{ $s->id }}" data-nama="{{ $s->nama }}" {{ old('satuan_dasar_id', $produk->satuan_dasar_id) == $s->id ? 'selected' : '' }}>{{ $s->nama }}</option>
                        @endforeach
                    </select>
                </div>
                
                {{-- STOK SAAT INI TIDAK ADA DI SINI. Sesuai instruksi (tidak bisa diedit lewat form) --}}
                
                {{-- STOK MINIMUM --}}
                <div>
                    <label class="block font-mono text-label-caps text-text-secondary mb-2">STOK MINIMUM PENGINGAT</label>
                    <input type="number" name="stok_minimum" value="{{ old('stok_minimum', (float)$produk->stok_minimum) }}" step="0.001" min="0" required
                           class="w-full px-4 py-3 bg-surface-container-low border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-all" />
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center">
                        ...
                        <input type="number" name="harga_modal_per_satuan_dasar" value="{{ old('harga_modal_per_satuan_dasar', (float)$produk->harga_modal_per_satuan_dasar) }}" min="0" required
            ...
            <div class="flex gap-4 justify-end mt-8 border-t border-table-border pt-6">
                ...
                <button type="submit" class="px-8 py-3 bg-primary text-on-primary font-bold rounded-xl hover:bg-primary-container shadow-sm active:scale-95 transition-all">Simpan Perubahan</button>
            </div>
        </form>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const dbSatuanJual = @json($produk->satuanJual);
            // Pakai old() jika validasi gagal, jika tidak pakai data DB
            const oldData = @json(old('satuan_jual')) || dbSatuanJual;
            ...
            // Init rows
            if (oldData && (Array.isArray(oldData) ? oldData.length > 0 : Object.keys(oldData).length > 0)) {
                // oldData from DB might be an array of objects, from old() it might be an object of objects
                const dataIter = Array.isArray(oldData) ? oldData : Object.values(oldData);
                dataIter.forEach(item => createRow(item));
            } else {
